...
static void (*IOReportIterate)(CFDictionaryRef, IOReportIterateBlock) = nullptr;
...
namespace {
	...
	bool load_ioreport_functions() {
		...
		return IOReportCopyChannelsInGroup and IOReportCreateSubscription and
		       ...
		       IOReportIterate and IOReportChannelGetChannelName and
	...
	std::pair<uint32_t, uint32_t> get_cpu_frequencies() {
		...
		if (g_last_sample != nullptr) {
			...
			if (delta != nullptr) {
				...
				__block uint32_t e_result = 0, p_result = 0;
				...
				IOReportIterate(delta, ^int(IOReportSampleRef sample) {
					try {
						...
						if (not is_ecpm and not is_pcpm) return kIOReportIterOk;

						const auto& freq_table = is_ecpm ? g_ecpu_freqs : g_pcpu_freqs;
						if (freq_table.empty()) return kIOReportIterOk;
						...
						if (total_residency > 0) {
							(is_ecpm ? e_result : p_result) = static_cast<uint32_t>(weighted_freq / total_residency);
						}
					} catch (...) {
						// Never allow exceptions to escape the C blocks callback.
					}
					return kIOReportIterOk;
				});
